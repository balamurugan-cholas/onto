...
export default function LibraryPage({ worker, email, onDownload, onSignOut }: { worker: string; email: string; onDownload: (platform: 'windows') => Promise<void>; onSignOut: () => Promise<void> }) {
  const [products, setProducts] = useState<Product[]>([]), [error, setError] = useState(''), [busy, setBusy] = useState(false)
  useEffect(() => { accountApi<{ products: Product[] }>(worker, '/library').then(r => setProducts(r.products)).catch(e => setError(e.message)) }, [worker])
  const download = async () => { setBusy(true); setError(''); try { await onDownload('windows') } catch (e) { setError(e instanceof Error ? e.message : 'Download failed.') } finally { setBusy(false) } }
  return <main className="library-page">
    ...
    {products.map(product => <article className="library-product" key={product.id}>
      ...
      <div className="library-downloads"><button disabled={busy} onClick={() => void download()}>{busy ? 'Preparing…' : 'Download for Windows'}</button></div>
    </article>)}
  </main>
}
